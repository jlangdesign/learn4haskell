...
-- Hm, don't think there's a way to return both values if they are equidistant
-- if the return type needs to be Int
closestToZero x y =
  if (abs x) > (abs y)
    then y
    else x
...
mid x y z
  | (x < y && y < z) || (z < y && y < x) || (y == z) || (y == x)  = y
  | (y < x && x < z) || (z < x && x < y) || (x == y) || (x == z) = x
  | otherwise = z
